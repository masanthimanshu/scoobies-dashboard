...
export const BasketSizeAov: React.FC<BasketSizeAovProps> = React.memo(
  ({ channels, metrics }) => {
    ...
    const storeAvgBasket = useMemo(() => {
      const totalOrders = metrics.totalOrders || 1;
      return metrics.totalUnitsSold > 0 && totalOrders > 0
        ? metrics.totalUnitsSold / totalOrders
        : 0;
    }, [metrics.totalUnitsSold, metrics.totalOrders]);
    ...
    const displayedChannels = useMemo(() => {
      return showAll ? validChannels : validChannels.slice(0, 8);
    }, [showAll, validChannels]);
    ...
    const openAovEditor = () => {
      setTempAovInput(
        effectiveBenchmarkAov
          ? Math.round(effectiveBenchmarkAov).toString()
          : "",
      );
                ...
                {maxAovChannel &&
                  ...
                  validChannels.length > 1 && (
                    <div className="mt-3 pt-2.5 border-t border-[#EBE5D9] flex items-center gap-1.5 text-[10px] font-bold text-[#5F7161]">
                      ...
                      <span>
                        Highest ticket size leader across all channels
                      </span>
                    </div>
                  )}
                {maxBasketChannel &&
